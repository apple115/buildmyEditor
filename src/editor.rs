...
impl Editor {
    ...
    fn search(&mut self) {
        ...
        let query = self
            .prompt(
                "Search(ESC to cocel,Arrows to navigate):",
                |editor, key, query| {
                    let mut moved = false;
                    match key {
                        Key::Right | Key::Down => {
    ...
    fn scroll(&mut self) {
        ...
        let witdh = self.terminal.size().width as usize;
        ...
        if x < offset.x {
            offset.x = x;
        } else if x >= offset.x.saturating_add(witdh) {
            offset.x = x.saturating_sub(witdh).saturating_add(1);
        }
    }
